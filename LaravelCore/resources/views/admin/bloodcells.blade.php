...
@section('content')
    <div class="page-heading">
        ...
        <section class="section">
            ...
            <div class="card">
                @if (!empty(Auth::user()->can(App\Models\User::READ_BLOODCELLS)))
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-striped key-table" id="bloodcell-table">
                                <thead>
                                    <tr>
                                        <th>Mã</th>
                                        <th>Thú cưng</th>
...
@push('scripts')
    <script>
        $(document).ready(function() {
            const table = $('#bloodcell-table').DataTable({
                processing: true,
                serverSide: true,
                orderCellsTop: true,
                ajax: {
                    url: `{{ route('admin.bloodcell') }}` + window.location.search
                },
                columns: [
            ...
            initDataTable('bloodcell-table', '4, 5')
        });

        $('.list-branches').change(function() {
            window.location.href = `{{ route('admin.bloodcell') }}?branch_id=${$(this).val()}`
        })
    </script>
@endpush
