...
@section('content')
    <div class="page-heading">
        ...
        <section class="section">
            ...
            <div class="card">
                @if (!empty(Auth::user()->can(App\Models\User::READ_PRESCRIPTIONS)))
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-striped key-table" id="prescription-table">
                                <thead>
                                    <tr>
                                        <th>Mã</th>
                                        <th>Thú cưng</th>
...
@push('scripts')
    <script>
        config.routes.batchRemove = `{{ route('admin.prescription.remove') }}`

        $(document).ready(function() {
            const table = $('#prescription-table').DataTable({
                processing: true,
                serverSide: true,
                orderCellsTop: true,
                ajax: {
                    url: `{{ route('admin.prescription') }}` + window.location.search
                },
                columns: [
            ...
            initDataTable('prescription-table', '5, 6')
        })
        $(document).on('click', '.btn-prescription-refund', function(e) {
            e.preventDefault();
        ...
        $('.list-branches').change(function() {
            window.location.href = `{{ route('admin.prescription') }}?branch_id=${$(this).val()}`
        })
    </script>
@endpush
