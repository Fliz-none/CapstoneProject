...
@section('content')
    <div class="page-heading">
        ...
        <section class="section">
            ...
            <div class="card">
                @if (!empty(Auth::user()->can(App\Models\User::READ_ULTRASOUNDS)))
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-striped key-table" id="ultrasound-table">
                                <thead>
                                    <tr>
                                        <th>Mã</th>
                                        <th>Thú cưng</th>
...
@push('scripts')
    <script>
        $(document).ready(function() {
            const table = $('#ultrasound-table').DataTable({
                processing: true,
                serverSide: true,
                orderCellsTop: true,
                ajax: {
                    url: `{{ route('admin.ultrasound') }}` + window.location.search
                },
                columns: [
            ...
            initDataTable('ultrasound-table', '4, 5')
        });
        
        $('.list-branches').change(function() {
            window.location.href = `{{ route('admin.ultrasound') }}?branch_id=${$(this).val()}`
        })
    </script>
@endpush
