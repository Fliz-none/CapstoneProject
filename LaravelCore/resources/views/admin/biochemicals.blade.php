...
@section('content')
    <div class="page-heading">
        ...
        <section class="section">
            ...
            <div class="card">
                @if (!empty(Auth::user()->can(App\Models\User::READ_BIOCHEMICALS)))
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-striped key-table" id="biochemical-table">
                                <thead>
                                    <tr>
                                        <th>Mã</th>
                                        <th>Thú cưng</th>
...
@push('scripts')
    <script>
        $(document).ready(function() {
            const table = $('#biochemical-table').DataTable({
                processing: true,
                serverSide: true,
                orderCellsTop: true,
                ajax: {
                    url: `{{ route('admin.biochemical') }}` + window.location.search
                },
                columns: [
            ...
            initDataTable('biochemical-table', '4, 5')
        });
        $('.list-branches').change(function() {
            window.location.href = `{{ route('admin.biochemical') }}?branch_id=${$(this).val()}`
        })
    </script>
@endpush
