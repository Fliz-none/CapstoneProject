@php
    ...
    $pet = $info->_pet;
    $customer = optional($info->_pet)->_customer;
@endphp
        ...
        <div class="container content">
            <div class="row mb-3">
                ...
                <div class="col-4 ms-auto">
                    <p class="mb-0">Mã phiếu: <strong>{{ $info->code }}</strong></p><br>
                    <p class="mb-0">Ngày tạo: {{ $info->created_at->format('d/m/Y H:i') }}</p>
                    <div class="d-flex align-items-center justify-content-start" style="height: 15mm; overflow: hidden;">
                        <svg id="barcode-{{ $info->code }}"></svg>
                        <input class="barcode-value" type="hidden" value="{{ $info->code }}">
                    </div>
                </div>
            </div>
            <div class="mb-3 pt-3 text-center border-top">
                <h4 class="text-uppercase">PHIẾU KHÁM LÂM SÀNG</h4>
            </div>
            <div class="row mb-2">
                <div class="col-12 col-md-5 mb-2">
                    <small class="mb-0 pe-2">Thú cưng: <strong>{{ $pet ? $pet->name : '' }}</strong></small><br />
                    <small class="mb-0 pe-2">Loài: <strong>{{ $pet ? $pet->animal->specie . ' (' . ($pet->genderStr != '' ? $pet->genderStr : 'Không rõ giống') . ')' : '' }}</strong></small><br />
                    <small class="mb-0 pe-2">Tuổi: <strong> {{ $pet ? $pet->age ?? 'Không rõ' : '' }} </strong></small><br />
                    <small class="mb-0 pe-2">Triệt sản: <strong>{{ $pet ? ($pet->neuterStr != '' ? $pet->neuterStr : 'Không rõ') : '' }} </strong></small><br />
                </div>
                <div class="col-12 col-md-7 mb-2">
                    <small class="mb-0">Khách hàng: <strong>{{ $customer ? $customer->name : '' }}</strong></small><br />
                    <small class="mb-0">Điện thoại: <strong>{{ $customer ? $customer->phone ?? 'Không có' : '' }}</strong></small><br />
                    <small class="mb-0">Địa chỉ: <strong>{{ $customer ? $customer->address ?? 'Không có' : '' }}</strong></small><br />
                </div>
            </div>
            ...
            <div class="row border-top justify-content-between my-3">
                ...
                <div class="col-5 pt-3 text-center">
                    ...
                    <p class="mb-5">
                        <em><strong>{{ optional($info->_doctor)->name }}</strong></em>
                    </p>
                </div>
            </div>
        </div>
