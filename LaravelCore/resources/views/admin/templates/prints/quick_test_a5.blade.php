...
<div id="print-container" style="font-size: 75%; color: #000000">
        ...
        <div class="container content">
            <div class="row mb-3">
                ...
                <div class="col-4 ms-auto">
                    <p class="mb-0">Mã phiếu: <strong>{{ $quicktest->code }}</strong></p>
                    <p class="mb-0">Phiếu khám: <strong>{{ $info->code }}</strong></p>
                    <p class="mb-0">Ngày tạo: {{ $quicktest->created_at->format('d/m/Y H:i') }}</p>
            ...
            <div class="my-4 pt-3 text-center border-top">
                <h4 class="text-uppercase">PHIẾU KIT TEST NHANH</h4>
            </div>
            <div class="row my-3">
            ...
            <div class="row">
                ...
                <div class="col-12">
                    <h5 class="text-uppercase">THÔNG TIN PHIẾU KIT TEST NHANH </h5>
                </div>
            </div>
            <div class="row justify-content-between">
                <div class="col-12">
                    <p class="mb-0">Nội dung: <strong> {{ $quicktest->detail->_service->name }}</strong></p>
                    <p class="mb-0">Mẫu kiểm tra: <strong> {{ $quicktest->sample }}</strong></p>
                    <p class="mb-0">Phương pháp kiểm tra: <strong> {{ $quicktest->method }}</strong></p>
                    <p class="mb-0">Kết luận: <strong> {{ $quicktest->conclusion }}</strong></p>
                    <p class="mb-0">Khuyến cáo: <strong> {{ $quicktest->recommendation }}</strong></p>
                </div>
            </div>
            ...
            <div class="row border-top justify-content-between my-3">
                ...
                <div class="col-5 pt-3 text-center">
                    ...
                    <p class="mb-5">
                        <em><strong>{{ $quicktest->technician_id ? $quicktest->technician->name : '' }}</strong></em>
                    </p>
                </div>
            </div>
        </div>
    ...
    @if ($quicktest->images)
        @foreach ($quicktest->galleryUrl as $e => $gallery)
            <div class="text-center py-5 w-100 h-100" style="page-break-before: always;">
                <img src="{{ $gallery }}" alt="Ảnh {{ $e + 1 }}" style="max-width: 20cm; max-height: 29cm; object-fit: cover;">
            </div>
        @endforeach
    @endif
</div>
