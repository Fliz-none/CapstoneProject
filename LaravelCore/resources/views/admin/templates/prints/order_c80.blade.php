...
<div id="print-container" style="font-size: 75%; color: #000000">
    <div class="container-fluid print-template">
        ...
        <div class="row mb-3 pb-3" style="border-bottom: 1px solid #000;">
            ...
            @if ($order->customer_id)
                ...
                <div class="col-6"><small>Số điện thoại:</small></div>
        ...
        @if ($goods->count())
            ...
            <div class="row">
                @foreach ($goods as $i => $good)
                    ...
                    <div class="col-8">
                        <small>{{ $good->quantity . ' ' . $good->_unit->term }} &times; {{ number_format($good->realPrice) . 'đ' }} {{ $good->discount ? ' - GIẢM ' . parseDiscount($good->discount) : '' }}</small>
                    </div>
        ...
        <div class="row pt-3 mt-3" style="border-top: 1px solid #000;">
            ...
            @if ($order->discount > 0 || $order->saveAmount > 0)
                ...
                <div class="col-5 text-end fw-bold">
                    {{ $order->saveAmount > 0 ? number_format($order->saveAmount) . 'đ' : '' }}
                </div>
                @if ($order->saveAmount > 0)
                    ...
                    <div class="col-5 text-end">
                        {{ number_format($order->details->sum('originalTotal') - $order->details->sum('total')) }}đ
                    </div>
                @endif
                @if ($order->discount > 0)
                    ...
                    <div class="col-5 mb-3 text-end">
                        {{ number_format($order->details->sum('total') - $order->total) }}đ
                    </div>
                @endif
            @endif
            <div class="col-7 mb-3 fw-bold">
    ...
    @endif
    <div class="row mt-3">
        <div class="col-12 text-center">CẢM ƠN QUÝ KHÁCH VÀ HẸN GẶP LẠI<br />HOTLINE: {{ $settings['company_hotline'] }}<br /><small>Vui lòng đổi trả trong 24 giờ nếu hàng hóa có vấn đề</small></div>
    </div>
</div>
