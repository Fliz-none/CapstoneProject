        ...
        <div class="container content">
            <div class="row mb-3">
                ...
                <div class="col-4 ms-auto">
                    <p class="mb-0">Mã phiếu: <strong>{{ $biochemical->code }}</strong></p>
                    <p class="mb-0">Phiếu khám: <strong>{{ $info->code }}</strong></p>
                    <p class="mb-0">Ngày tạo: {{ $biochemical->created_at->format('d/m/Y H:i') }}</p>
            ...
            <div class="my-4 pt-3 text-center border-top">
                <h4 class="text-uppercase">Phiếu Xét Nghiệm Sinh Hóa Máu</h4>
            </div>
            <div class="row my-3">
            ...
            <div class="row">
                ...
                <div class="col-12">
                    <h5 class="text-uppercase">Thông Tin Phiếu Xét Nghiệm Sinh Hóa Máu</h5>
                </div>
            </div>
            <div class="row justify-content-between">
                <div class="col-12">
                    <p class="mb-0">Nội dung: <strong> {{ $biochemical->detail->_service->name }}</strong></p>
                    <table class="table">
                        <thead>
                            <tr>
                                <th>Chỉ tiêu</th>
                                ...
                                @foreach (json_decode($biochemical->details ?? [], true) as $index => $detail)
                                    <tr>
                                        @php
                                            ...
                                            $normal_index_str = $detail['normal_index'] ? $normal_index[0] . ' - ' . $normal_index[1] : '';
                                        @endphp
                                        <td><strong><span>{{ $index + 1 }}</span>. {{ $detail['name'] }}</strong> • {{ $normal_index_str }} {{ $detail['unit'] ? '(' . $detail['unit'] . ')' : '' }}
            ...
            <div class="row justify-content-between">
                <div class="col-12">
                    <p class="mb-0">Kết luận: <strong>{!! nl2br($biochemical->conclusion) !!}</strong></p>
                    <p class="mb-0">Khuyến cáo: <strong>{!! nl2br($biochemical->recommendation) !!}</strong></p>
                </div>
            </div>
            ...
            <div class="row border-top justify-content-between my-3">
                ...
                <div class="col-5 pt-3 text-center">
                    ...
                    <p class="mb-5">
                        <em><strong>{{ optional($biochemical->technician)->name }}</strong></em>
                    </p>
                </div>
            </div>
        </div>
    </div>
    @if ($biochemical->images)
