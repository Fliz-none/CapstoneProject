...
<div class="modal-dialog modal-lg">
    <div class="modal-content">
        <div class="modal-header">
            <h1 class="modal-title fs-5" id="xray-modal-label">Phiếu X-Quang {{ $xray->code }}</h1>
            ...
            <div class="container">
                ...
                <div class="row">
                    <div class="col-12 mb-3">
                        ...
                        <p class="mb-0">KTV thực hiện: <strong>{{ $xray->technician_id ? $xray->technician->name : '' }}</strong></p>
                    </div>
                    <div class="col-12 text-center">
                        <h5 class="text-uppercase">Phiếu X-Quang {{ $xray->code }}</h5>
                    </div>
                </div>
                <div class="row justify-content-between">
                    <div class="col-12">
                        <p class="border-bottom pb-2">Nội dung: <strong>{{ $xray->detail->_service->name }}</strong></p>
                        @if ($xray->details)
                                            ...
                                            <div class="col-12 col-lg-6 mb-2">
                                                ...
                                                @if (isset($detail['images']))
                                                    <div class="row align-items-center border border-light-subtle rounded-1 m-0">
                                                        @foreach ($xray->galleryUrl[$index] as $e => $gallery)
                                                ...
                                                @else
                                                    <p>(Chưa có hình ảnh được thêm)</p>
                                                @endif
                                            </div>
                                            <div class="col-12 col-lg-6 mb-2">
                ...
                <p class="border-top py-2">Kết luận: <strong> {{ $xray->conclusion }} </strong></p>
            </div>
        </div>
        <div class="modal-footer">
            <button class="btn btn-primary text-decoration-none btn-print print-xray" data-id="{{ $xray->id }}" data-url="{{ getPath(route('admin.xray')) }}" type="button">
                <i class="bi bi-printer-fill"></i> In phiếu
            </button>

            @if (!empty(Auth::user()->hasAnyPermission(App\Models\User::UPDATE_XRAY)))
                <button class="btn btn-primary text-decoration-none btn-update-xray" data-bs-dismiss="modal" data-id="{{ $xray->id }}" type="button" aria-label="Close">
