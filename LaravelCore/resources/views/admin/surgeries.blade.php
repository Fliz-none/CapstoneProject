...
@section('content')
    <div class="page-heading">
        ...
        <section class="section">
            ...
            <div class="card">
                @if (!empty(Auth::user()->can(App\Models\User::READ_SURGERIES)))
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-striped key-table" id="surgery-table">
                                <thead>
                                    <tr>
                                        <th>Mã</th>
                                        <th>Thú cưng</th>
...
@push('scripts')
    <script>
        $(document).ready(function() {
            const table = $('#surgery-table').DataTable({
                bStateSave: true,
                stateSave: true,
                serverSide: true,
                orderCellsTop: true,
                ajax: {
                    url: `{{ route('admin.surgery') }}` + window.location.search
                },
                columns: [
            ...
            initDataTable('surgery-table', '4, 5')
        })
        $('.list-branches').change(function() {
            window.location.href = `{{ route('admin.surgery') }}?branch_id=${$(this).val()}`
        })
    </script>
@endpush
