    ...
    <li class="p-2 border-bottom bg-body-tertiary {{ $active_id == $conversation->id ? 'active' : '' }}" data-conversation_id="{{ $conversation->id }}">
        <a class="d-flex justify-content-between" href="javascript:void(0);">
            <div class="d-flex flex-row">
                @php
                    $obj = $conversation->customer ?? $conversation->created_user;
                @endphp
                <img class="rounded-circle d-flex align-self-center me-3 shadow-1-strong" src="{{ $obj->avatarUrl }}" alt="avatar" width="60" height="60">
                <div class="pt-1 w-100">
                    <p class="fw-bold mb-0">{{ $conversation->name ?? $obj->name }}</p>
...
@empty
    <li class="p-2 border-bottom bg-body-tertiary" data-conversation_id="0">
        <div class="d-flex flex-row">
            <img class="rounded-circle d-flex align-self-center me-3 shadow-1-strong" src="{{ asset('admin/images/placeholder.webp') }}" alt="avatar" width="60" height="60">
            <div class="pt-1">
