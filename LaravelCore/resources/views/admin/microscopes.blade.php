...
@section('content')
    <div class="page-heading">
        ...
        <section class="section">
            ...
            <div class="card">
                @if (!empty(Auth::user()->can(App\Models\User::READ_MICROSCOPES)))
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-striped key-table" id="microscope-table">
                                <thead>
                                    <tr>
                                        <th>Mã</th>
                                        <th>Thú cưng</th>
...
@push('scripts')
    <script>
        $(document).ready(function() {
            const table = $('#microscope-table').DataTable({
                processing: true,
                serverSide: true,
                orderCellsTop: true,
                ajax: {
                    url: `{{ route('admin.microscope') }}` + window.location.search
                },
                columns: [
            ...
            initDataTable('microscope-table', '4, 5')
        });
        $('.list-branches').change(function() {
            window.location.href = `{{ route('admin.microscope') }}?branch_id=${$(this).val()}`
        })
    </script>
@endpush
